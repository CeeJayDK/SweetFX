...
// !!! tabbing func contents out to make it easier to
...
float3 AsciiPass( float2 tex )
{
	...
	*/
	...
	// !!! pre-calc texcoords w/ offsets
	// !!! to reduce overall calculations
...
//	float2 cp65 = cursor_position + BUFFER_PIXEL_SIZE * 6.5;
	...
	// !!! piece together texcoords using pre-canned vals from above
	float3 color;
	...
	*/
	...
	{	
		...
		*/
		...
		// !!! taking it further, we can
		// !!! put values in array & use
		// !!! step function to do quant compare
...
//		int	index = charindex( quant, gray, 16 );
	...
	else // Ascii_font == 0 , the 3x5 font
	{
		...
		*/
		...
		// !!! using step() to sift through quants
...
//		character = 0.0; 																					//If so make the pixel black.
	...
	// !!! pre-calc'ing clamp min/max to tidy up a bit
